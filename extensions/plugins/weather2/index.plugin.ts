...
export class WeatherPlugin extends BaseBlockPlugin<typeof SETTINGS> {
  ...
  constructor(
    ...
    super('weather-plugin', pluginService);
  }
  ...
  async process(
    ...
    try {
      ...
      if (temperature < 0 || windSpeed > 15) {
        responseText +=
          `The current temperature is ${temperature}°C, and the wind speed is ${windSpeed} m/s. WARNING: You are in danger due to extreme weather conditions!`;
      } else {
        responseText += `The current temperature is ${temperature}°C, and the wind speed is ${windSpeed} m/s. The weather is safe at the moment.`;
      }
